...
final class CountryCodeTests: XCTestCase {
    ...
    func testCountryCalledCode() throws {
        let country: Country = .init(code: "DE", label: "Germany", callingCode: 49)
        
        XCTAssertEqual(country.code, "DE")
        XCTAssertEqual(country.label, "Germany")
    ...
    func testArrayExtensions() throws {
        ...
        XCTAssertEqual(countries.country(forCode: "AR")?.states?.first?.label, "Buenos Aires")
    }
}
